...
async function denoify(params) {
    ...
    {
        const includes = ((_d = denoifyParamsFromPackageJson.includes) !== null && _d !== void 0 ? _d : ["README.md", "LICENSE"])
        ...
        objIncludes
            .forEach(({ src, destDir, destBasename }) => (0, fsCopy_1.fsCopy)(src, path.join(denoDistPath, path.join(destDir !== null && destDir !== void 0 ? destDir : path.dirname(src), destBasename !== null && destBasename !== void 0 ? destBasename : path.basename(src)))));
    }
}
...
function getIndexFileRelativePath(params) {
    ...
    const indexFileRelativePath = (0, toPosix_1.toPosix)(path.relative(tsconfigOutDir, path.normalize(packageJsonParsed["main"]) // ./dist/lib/index.js
    ) // ./lib/index.js
        .replace(/\.js$/i, ".ts"));
